...
- **Doctor / report / OTel export:** `node scripts/doctor.mjs` checks the harness's own wiring (dist built, hooks wired, all 5 MCP servers boot, policy parses) and exits non-zero on a problem; `node scripts/report.mjs` renders the journal/ledger/memory into one static HTML page; `node scripts/otel-export.mjs` maps the guard journal onto OTLP/HTTP JSON spans (hand-rolled, zero new deps) and POSTs to `OTEL_EXPORTER_OTLP_ENDPOINT` or writes a file if unset — see `decisions.md` D040 for why this lives in `scripts/` and not inside `src/guard/`.
